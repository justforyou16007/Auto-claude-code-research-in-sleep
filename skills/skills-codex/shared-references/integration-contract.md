...
| `analysis_tools.py` `register` / `deprecate` / `add-category` (in `/analyse-tool`) | B (side-effect) | Personal convenience collection of reusable analysis methods (`$HOME/.aris/analysis_tools/`, overridable via `$ARIS_ANALYSIS_TOOLS_DIR`); primary research output delivered without it (warn-and-skip). Personal — not repo-local — because skills cannot be reloaded at runtime so the helper *is* the access mechanism. Register enforces mandatory `scripts/tool-unit-test.py` + `references/` test-data contract; `test --slug` is the Type-A deterministic gate the subagent runs first. |
| `analysis_tools.py` `list` / `query` / `get` / `load` / `resource` / `test` / `categories` / `stats` (in `/analyse-tool`) | B (side-effect) | Read-only / execution access; unresolved ⇒ warn-and-skip. `load`/`resource` provide 3-tier progressive disclosure (L0 metadata → L1 full SKILL.md → L2 per-script/reference); `test` runs `tool-unit-test.py` and appends a `test` ledger receipt. All of `load`/`register`/`merge`/`run` fan out to a subagent (`spawn_agent`/`send_input`) — only `find` runs in the main agent. |
...
| Experiment env control | `gpu:` in AGENTS.md / CLAUDE.md | `experiment_env/env_helper.py` provision/preflight/sync/deploy/monitor/collect/destroy → A (gate); parse/info → E (diagnostic) | `.aris/experiment-env.json` | Step in run-experiment / monitor / vast-gpu / serverless-modal / experiment-queue | Re-run parse (agent re-reads AGENTS.md/CLAUDE.md → candidate JSON → `env_helper.py parse`) | env_helper exits 0/1/10-16 |
