...
> **Load this role, then execute it.** This file is the *whole* find-or-create-
...
>
> You are a **Codex** fan-out subagent. The analysis *work* is yours; the
> *verification* (the tool's test set + unit test) is authored by a **different
...
`test --slug` *run* (exit 0 / non-zero) is a Type-A gate that is safe for you
to execute (canonical `shared-references/acceptance-gate.md` in the main tree:
a shell exit code is execution bookkeeping, not a verdict); the *authorship* of
...
1. **`query`** for same/similar tools (L0) — `query --query "<analysis shape>"
   [--category <k>] [--inputs <a,b>]`. This is the *find* step: it is cheap
...
2. **If a match exists** → `load` **only that one** candidate (L1). Follow the
   ...
   analysis, return the result. (If the loaded tool is *almost* right but
...
4. **`test --slug <s>`** → must pass (exit 0). This is the 验证效果 gate — a
   ...
   in `registry.jsonl`. If it fails, **fix the *work* (your script / spec)**,
...
substitute for the cross-model authoring rule: it means *no* test exists, not
...
- Canonical `shared-references/acceptance-gate.md` and
  ...
  `test` gate, safe same-model to *run*) vs Type-B (promoting a tool into the
