...
The reusable-method registry is the *mechanism*, not the product. The product is
...
method (often *with* the script it used), other agents had no way to discover /
...
1. **No runtime skill reload.** Codex CLI cannot reload skills mid-session — a
   ...
   restart. So this helper *is* the access mechanism: an agent `load`s a
...
*spawned* (`spawn_agent`/`send_input` here; `Agent` tool in the Claude Code
