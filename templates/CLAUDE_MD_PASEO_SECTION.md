...
executor_mode:          auto               # auto | bypassPermissions (overnight) | plan
...
# --- Dispatch / lifecycle ---
notify_on_finish:       true               # push model; do NOT wait_for_agent
...
| `executor_mode` | `auto` | `auto` (workspace-write, on-request approvals) · `bypassPermissions` (overnight, no approval round-trips) · `plan` (read-only planning, never for a phase that writes artifacts). |
...
| `reviewer_mode` | `full-access` | codex agent sandbox: `full-access` (autonomous repo read, network on, `codex exec` analog — the default) · `auto` (workspace-write, on-request) · `read-only` (cannot write the verdict file — do not use for reviewers). `auto-review` is Codex's *internal* guardian, NOT our reviewer. |
...
| `notify_on_finish` | `true` | Push model: children notify the parent on completion; the parent does NOT `wait_for_agent`. Keep `true`. |
...
- `skills/shared-references/paseo-subagent-dispatch.md` — executor sub-agent spawn shape, two continuity modes, fanout discipline, lifecycle.
- `skills/shared-references/paseo-reviewer-dispatch.md` — codex reviewer spawn shape, fresh-vs-continuation rule, `save_trace.sh` `--thread-id` contract.
